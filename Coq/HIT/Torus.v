...
Structure Torus := {
  ...
  nd_compute_base :
    (forall P pt lp1 lp2 f,
      nd_torus_rect P pt lp1 lp2 f base == pt);
  nd_compute_loop1 :
    (forall P pt lp1 lp2 f,
      map (nd_torus_rect P pt lp1 lp2 f) loop1 ==
      nd_compute_base P pt lp1 lp2 f @ lp1 @ !(nd_compute_base P pt lp1 lp2 f));
  nd_compute_loop2 :
    (forall P pt lp1 lp2 f,
      map (nd_torus_rect P pt lp1 lp2 f) loop2 ==
      nd_compute_base P pt lp1 lp2 f @ lp2 @ !(nd_compute_base P pt lp1 lp2 f));
  torus_rect :
    ...
    (forall (P : torus -> Type) (pt : P base) 
            ...
            (f : transport (P := (fun x => transport x pt == pt)) face
                 ...
                 (happly_dep (transport_comp P loop2 loop1) _ @
                  (map (transport loop2) lp1) @
                  lp2)),
     torus_rect P pt lp1 lp2 f base == pt);
  compute_loop1 :
    (forall P pt lp1 lp2 f,
      map_dep (torus_rect P pt lp1 lp2 f) loop1
      == map (transport loop1) (compute_base P pt lp1 lp2 f) @ lp1 @ !compute_base P pt lp1 lp2 f);
  compute_loop2 :
    (forall P pt lp1 lp2 f,
      map_dep (torus_rect P pt lp1 lp2 f) loop2
      == map (transport loop2) (compute_base P pt lp1 lp2 f) @ lp2 @ !compute_base P pt lp1 lp2 f)
}.

Implicit Arguments base [t].
